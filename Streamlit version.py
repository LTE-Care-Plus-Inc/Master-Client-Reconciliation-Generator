...
st.markdown(
    """
This tool creates a **single master client list** by reconciling  
**Aloha**, **Zoho**, and **HiRasmus** using **Insurance ID + fuzzy name matching**.
"""
)
...
# FILE UPLOADS
# =========================
aloha1_file = st.file_uploader("Upload Aloha1.xlsx (Appointments)", type=["xlsx"])
aloha2_file = st.file_uploader("Upload Aloha2.xlsx (Client Roster)", type=["xlsx"])
zoho_file = st.file_uploader("Upload Zoho.xlsx (Inactive Status)", type=["xlsx"])
hirasmus_file = st.file_uploader("Upload HiRasmus.xlsx (Client Status)", type=["xlsx"])
...
if run_btn:
    ...
    # DOWNLOAD
    # =========================
    output_bytes = final.to_excel(index=False)
    st.download_button(
